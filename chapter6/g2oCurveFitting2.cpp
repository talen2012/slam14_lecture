...
public:
    ...
    virtual void computeError() override
    {
        ...
        const Eigen::Vector4d abc = v->estimate(); // BaseVertex类的estimate()方法获取当前优化参数值
        // _measurement通过setMeasurement()传入，_measurement和_error是BaseEdge的成员变量
        // setMeasurement()是BaseEdge的成员函数
        // e = y - (exp(a*x*x + b*x +c) + d)
        _error(0, 0) = _measurement - (exp(abc(0, 0) * _x * _x + abc(1, 0) * _x + abc(2, 0)) + abc(3, 0) * _x);
    }

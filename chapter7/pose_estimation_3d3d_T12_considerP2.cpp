...
/** 本程序在pose_estimation_3d3d_T12的基础上
 *  将相机2坐标下空间点的位置，也视为待优化量
 *  新增了一个顶点类型VertexPoint
 *  将边类型改成了二元边
*/
...
// unary edge
// 将边类型修改为二元边
class EdgePosition : public g2o::BaseBinaryEdge<3, Eigen::Vector3d, VertexPose, VertexPoint> // 模板参数<观测值维度，观测值类型，连接顶点1类型, 连接顶点2类型>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW; // 解决Eigen库数据结构内存对齐问题
    ...
    virtual void computeError() override
    {
        ...
        Eigen::Vector3d pt_3d_aftertrans = T * vj->estimate(); // 将相机2坐标下的空间点位置转换到相机1下(相机1坐标系视为世界坐标系)
        // _measurement和_error是BaseEdge的成员变量
        // _measurement通过setMeasurement()传入，setMeasurement()是BaseEdge的成员函数
        _error = _measurement - pt_3d_aftertrans; // 观测值为空间点在相机1坐标(世界坐标)下的位置
    }
...
void bundleAdjustment(
    const VecVector3d &pts1,
    VecVector3d &pts2, // 这里相机2坐标下空间点的位置不再作为固定量，而是作为待优化量的初值传入
    Mat &R, Mat &t);
                          ...
                          Mat &R, Mat &t)
{
    ...
    // compute q2*q1^T
    // 注意，十四讲源码里计算的是q1*q2^T，即第二帧到第一帧的变换
    ...
    Mat &R, Mat &t)
{
    ...
    for (size_t i = 0; i < pts1.size(); i++)
    {
        ...
        edge_position->setVertex(1, dynamic_cast<VertexPoint *>(optimizer.vertex(i + 1))); // 前文已在优化图中添加了顶点，因此可以使用顶点ID访问
    ...
    {
        pts2[i] = (dynamic_cast<VertexPoint *>(optimizer.vertex (i+1)))->estimate();
    }
}
...
int main(int argc, char **argv)
{
    ...
    // // 验证pt2 = R * pt1 + t
    cout << "验证pt2 = R * pt1 + t: " << endl;
    for (int i = 0; i < 5; i++)
    {
        cout << "pt1 = " << pts1_3d[i].transpose() << endl;
        cout << "pt2 = " << pts2_3d[i].transpose() << endl;
        cout << "R * pt1 + t = " << (R * Vec3d(pts1_3d[i][0], pts1_3d[i][1], pts1_3d[i][2]) + t).t() << endl;
        cout << endl;
    }
                          ...
                          Mat &R, Mat &t)
{
    ...
    // compute q2*q1^T
    // 注意，十四讲源码里计算的是q1*q2^T，即第二帧到第一帧的变换，但这里我计算的是第一帧到第二帧的变换
    ...
    Mat &R, Mat &t)
{
    ...
    for (size_t i = 0; i < pts1.size(); i++)
    {
        auto pt1 = pts1[i];
        auto pt2 = pts2[i];
        EdgePosition *edge_positon = new EdgePosition(pt1);
        edge_positon->setVertex(0, vertex_pose);
        edge_positon->setMeasurement(pt2); // 设定观测值

...
int main(int argc, char **argv)
{
    ...
    for (DMatch match : matches)
    {
        ...
        pts_3d.push_back(Point3f(p1_cam.x * dd, p1_cam.y * dd, dd)); // 考虑相机1的位姿为未知时，将这里获得的pts_3d视为世界坐标系下的3d点
        pts1_2d.push_back(p1_pixel); // 新增，定义容器记录图像1关键点的像素坐标
        pts2_2d.push_back(p2_pixel);
    }
    ...
    bundleAdjustmentSelfGaussNewton(pts_3d_eigen, pts2_2d_eigen, K, pose_gn);
...
void bundleAdjustmentG2O(
    const VecVector3d &points_3d,
    const VecVector2d &points1_2d, // 新增传入相机1图像关键点的像素坐标，即世界坐标3d点在相机1中的投影
    const VecVector2d &points2_2d,
    const Mat &K,
    Sophus::SE3d &pose)
{
    ...
    // -- 第三步：设定优化图结构
    // 1.1 新增：添加顶点0，优化变量是相机1的位姿
    VertexPose *vertex_pose0 = new VertexPose(); // camera1 vertex_pose
    vertex_pose0->setId(0);
    vertex_pose0->setEstimate(Sophus::SE3d()); // 优化变量赋初值
    optimizer.addVertex(vertex_pose0);
    // 1.2 添加顶点1，优化变量是相机2的位姿
    VertexPose *vertex_pose1 = new VertexPose(); // camera2 vertex_pose
    vertex_pose1->setId(1);
    vertex_pose1->setEstimate(Sophus::SE3d()); // 优化变量赋初值
    ...
    cout << "pose2 estimated by g2o =\n"
         << vertex_pose1->estimate().matrix() << endl;
    pose = vertex_pose1->estimate() * vertex_pose0->estimate().inverse(); // 这里输出的是相机1到相机2的相对位姿

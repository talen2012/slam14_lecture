// 本程序是视觉SLAM14讲第7讲课后习题7的解答
// 将空间点在相机2下的坐标不再视为固定已知量，而是作为待优化量的初值传入
// 因此将该待优化量设置为一个顶点类型，并连接到误差边
// 原误差边由一元边变为二元边
...
#include <g2o/core/base_binary_edge.h>                    // G2O单边基类，自定义单边需要继承该类，并创建有参构造函数，重载computeError()计算残差函数和linearizeOplus()计算雅各比矩阵函数
...
#include <g2o/core/optimization_algorithm_levenberg.h>    // G2O优化算法，列文伯格
...
// 2. 将原来的单边修改为二元边，两个顶点为相机2到1的位姿和相机2下空间点坐标
class EdgePosition : public g2o::BaseBinaryEdge<3, Eigen::Vector3d, VertexPose, VertexPoint> // 模板参数<观测值维度，观测值类型，连接顶点类型>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW; // 解决Eigen库数据结构内存对齐问题
    ...
    {
        // _vertices是hyper_graph中Edge类的成员，由setVertex(index, vertex指针)成员函数指定，最终被BaseBinaryEdge继承
        VertexPose *vi = static_cast<VertexPose *>(_vertices[0]); // 基类指针转换为派生类指针，可能会越界访问，除非你知道自己在干什么
        Sophus::SE3d T = vi->estimate();
        VertexPoint *vj = static_cast<VertexPoint *>(_vertices[1]);
        Eigen::Vector3d pt_3d_aftertrans = T * (vj->estimate());
...
void bundleAdjustment(
    const VecVector3d &pts1,
    VecVector3d &pts2, // 这里pts2不再作为一个固定已知量，而是作为一个待估计量的初值传入
    Mat &R, Mat &t)
{
    // -- 第一步：设定优化算法
    // 1. 定义一个块求解器类型
    ...
    auto optim_method = new g2o::OptimizationAlgorithmLevenberg(             // 注意：这里使用高斯牛顿法损失为无穷大，改用LM方法
    ...
    // -- 第三步：设定优化图结构
    // 1.1 添加位姿顶点VertexPose，只有一个
    VertexPose *vertex_pose = new VertexPose();
    vertex_pose->setId(0);
    vertex_pose->setEstimate(Sophus::SE3d()); // 优化变量赋初值
    optimizer.addVertex(vertex_pose);
    // 1.2 添加相机2中空间点的坐标顶点VertexPoint
    for (size_t i = 0; i < pts2.size(); i++)
    {
        VertexPoint *vertex_point = new VertexPoint(); 
        vertex_point->setId(i + 1);
        vertex_point->setEstimate(pts2[i]);
        optimizer.addVertex(vertex_point);
    }
    // 2. 添加边，误差 = 相机1中(即世界坐标下)空间点的坐标 - 相机2中空间点转换到相机1下的坐标
    for (size_t i = 0; i < pts1.size(); i++)
    {
        auto pt1 = pts1[i];
        // auto pt2 = pts2[i];
        EdgePosition *edge_position = new EdgePosition();
        edge_position->setVertex(0, vertex_pose);
        edge_position->setVertex(1, dynamic_cast<VertexPoint *>(optimizer.vertex(i + 1)));
        edge_position->setMeasurement(pt1); // 设定观测值
        edge_position->setInformation(Eigen::Matrix3d::Identity());
        optimizer.addEdge(edge_position);
    }
    ...
    for(size_t i = 0; i < pts2.size(); i++)
    {
        Eigen::Vector3d pt2_optim = (dynamic_cast<VertexPoint *>(optimizer.vertex(i + 1)))->estimate();
        pts2[i] = pt2_optim;
    }
}